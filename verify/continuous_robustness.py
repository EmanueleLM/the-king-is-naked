...
eps = 0.5  # min accuracy to test a model

# Test 
X = ['this @category@ movie is not @augment@ @positive@',
     'this @category@ movie is not @augment@ @negative@',
     '@gender@ @positiveverb@ @category@ movie',
     '@gender@ @negativeverb@ @category@ movie',
     'I think this @category@ film is @augment@ @positive@',
     'I think this @category@ film is @augment@ @negative@'
    ]
Y = [1, 0, 1, 0, 1, 0]
...
replace['@augment@'] = ['very', 'incredibly', 'super', 'extremely', 'greatly']
replace['@positive@'] = ['good', 'fantastic', 'nice', 'satisfactory', 'interesting', 'satisfying']
replace['@negative@'] = ['bad', 'poor', 'boring', 'terrible', 'awful', 'dreadful']
replace['@gender@'] = ['she', 'he', 'sarah', 'matthew', 'simone']
replace['@positiveverb@'] = ['liked', 'enjoyed', 'likes', 'enjoys', 'appreciates', 'appreciated']
replace['@negativeverb@'] = ['did not like', 'does not like', 'did not enjoy', 'does not enjoy', 'does not appreciate', 'did not appreciate']
label_changing_replacements = []

# generate some lp noise
def lp_noise(x, eps, norm='linf'):
    if norm == 'inf':
        return x+(np.random.rand(len(x),emb_dims)*eps)
    if norm == '1':
        pass
    if norm == '2':
        pass
...
for exp,f in enumerate(files):
    ...
    for x,y in zip(X_pert, Y_pert):
        ...
        x = lp_noise(x, eps, 'inf')  # add some lp noise
