...
X_test_emb = [[index2embedding[word2index[x]] for x in xx] for xx in X_test]
X_test_emb = np.asarray(pad_sequences(X_test_emb, maxlen=maxlen, emb_size=emb_dims))
X_test_emb = X_test_emb.reshape(len(X_test_emb), *input_shape[1:])
y_test_lables = to_categorical(y_test, num_classes=2)
...
for exp, m_name in enumerate(files_):
    ...
    _, accuracy = model.evaluate(X_test_emb, y_test_lables, batch_size=64)  # evaluate
    W = []
...
print(f"Average {architecture} accuracy on `{test_type}` over {len(X_test)} evaluations:: {np.mean(accuracies):.4f} \pm {np.std(accuracies):.4f}")
print(f"Weights norm: {np.mean(W_means):.4f} \pm {np.std(W_means):.4f}")

# # confirm results
# right, tot = 0, 0
# for x,y in zip(X_test_emb,y_test_lables): 
#     yy = np.argmax(model.predict(x.reshape(*input_shape))) 
#     if np.argmax(y) == yy: 
#         right += 1 
#     tot += 1 
# print(right/tot)
