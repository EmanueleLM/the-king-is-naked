...
def load_CLAUDETTE(maxlen):
    """
    maxlen is the number of words in each input text (eventually padded/cut)
    output:
     list of lists with train and test inputs (strings) and labels 
      (Unused: 0, Fair: 1, Potentially Unfair: 2, Clearly Unfair: 3, Unclassified: 4).
    """
    # Load CLAUDETTE dataset (Tokenize)
    X_train = read_csv('./data/datasets/CLAUDETTE/train/train.csv', sep=',',header=None).values
    X_test = read_csv('./data/datasets/CLAUDETTE/eval/test.csv', sep=',',header=None).values
    y_train, y_test = [], []
    for i in range(len(X_train)):
        r, s = X_train[i]  # review, score (comma separated in the original file)
        X_train[i][0] = r.split(' ')[:maxlen-2]
...
def dataset_to_dataloader(X, y, tokenizer, maxlen, batch_size=32, device=torch.device("cpu"), shuffle=True):
    """
    ...
    X_sampler = (RandomSampler(X_data) if shuffle is True else None)
